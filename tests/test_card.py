...
def test_card_creation():
    ...
    # Test red card
    hearts_ace = Card(suit=CardSuit.HEARTS, rank=CardRank.ACE)
    assert hearts_ace.is_red is True
    assert hearts_ace.is_black is False

    # Test black card
    spades_king = Card(suit=CardSuit.SPADES, rank=CardRank.KING)
...
def test_card_value_calculation():
    ...
    hearts_king = Card(suit=CardSuit.HEARTS, rank=CardRank.KING)
...
def test_card_color_conversion():
    ...
    # Test diamonds ↔ clubs
    diamonds_king = Card(suit=CardSuit.DIAMONDS, rank=CardRank.KING)
...
def test_card_pair_creation():
    ...
    assert black_pair.breakeven == "<=11"

    # Invalid pair - both small
    with pytest.raises(ValueError):
        CardPair(
            small_card=Card(suit=CardSuit.HEARTS, rank=CardRank.ACE),
            big_card=Card(suit=CardSuit.SPADES, rank=CardRank.SIX)
        )

    # Invalid pair - both big
    with pytest.raises(ValueError):
        CardPair(
            small_card=Card(suit=CardSuit.HEARTS, rank=CardRank.KING),
            big_card=Card(suit=CardSuit.SPADES, rank=CardRank.QUEEN)
        )
...
def test_breakeven_price():
    ...
    assert pair_black.breakeven == "<=12"
